...
const ProducerMessages = () => {
  ...
  const threadsQuery = useQuery<{ id: string; last_message_at: string; writer_id: string; }[]>({
    queryKey: ["producer-threads"],
    queryFn: async () => {
      const { data: { user }, error: userErr } = await supabase.auth.getUser();
      if (userErr) throw userErr; if (!user) throw new Error("Not authenticated");
      const { data, error } = await supabase
        .from("message_threads")
        .select("id, last_message_at, writer_id")
  ...
  const messagesQuery = useQuery<{ id: string; body: string | null; created_at: string; sender_id: string; }[]>({
    queryKey: ["producer-thread-messages", selectedThreadId],
  ...
  const sendMutation = useMutation({
    ...
    onSuccess: async () => {
      setDraft("");
      await qc.invalidateQueries({ queryKey: ["producer-thread-messages", selectedThreadId] });
      await qc.invalidateQueries({ queryKey: ["producer-threads"] });
    },
  });
    ...
    <div className="min-h-screen p-6">
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass-card p-4 md:col-span-1">
          ...
          <div className="space-y-2">
            {threadsQuery.data?.map((t) => (
              <button key={t.id} onClick={() => setSelectedThreadId(t.id)} className={`w-full text-left p-3 rounded-md border transition ${selectedThreadId === t.id ? "bg-accent" : "bg-background hover:bg-accent"}`}>
        ...
        <div className="glass-card p-4 md:col-span-2 flex flex-col h-[70vh]">
          ...
          <form className="mt-3 flex gap-2" onSubmit={(e) => { e.preventDefault(); sendMutation.mutate(); }}>
