  ...
  const { data: matches, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("matches")
        .select("id, created_at, score, state, writer_id, script_id")
  ...
  const openThread = useMutation({
    mutationFn: async (m: any) => {
      ...
      const { data: existing, error: findErr } = await supabase
        ...
        .select("id")
        .eq("writer_id", m.writer_id)
        .eq("producer_id", m.producer_id || (await supabase.auth.getUser()).data.user?.id)
        .limit(1)
        .maybeSingle();
    ...
    onSuccess: (threadId: string) => {
      navigate(`/producer/messages?threadId=${threadId}`);
    },
  });
